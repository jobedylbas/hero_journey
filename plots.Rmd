...
char_filename <- './data/archetypes/70160_characters.csv'
lk_re_filename <- './data/relations_types/70160_relations_types.csv'
...
deg <- max(centr_degree(g)$res)

sdd <- sd(centr_degree(g)$res)
mdd <- mean(centr_degree(g)$res)

sdb <- sd(centr_betw(g)$res)
mdb <- mean(centr_betw(g)$res)
g <- decompose.graph(g)[[1]]
sdc <- sd(centr_clo(g)$res)
mdc <- mean(centr_clo(g)$res)
